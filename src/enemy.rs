pub(super) mod prelude {
    pub(crate) use super::{Enemy, take_damage};
}
...
pub fn init(mut commands: Commands, mut enemy_ref: ResMut<Enemies>, position: Vec3) {
    enemy_ref.current_number += 1;
    commands
    .spawn(Enemy {is_deinit: false, health: enemy_ref.health})
    .insert(SpriteBundle { transform: Transform { translation: position + Vec3 {x: 30f32, y: 30f32, z: 0f32} / 2f32, scale: Vec3 {x: 30f32, y: 30f32, z: 0f32}, ..Default::default()}, sprite: Sprite { color: ColorPalette::GRAY, ..Default::default()}, ..Default::default()});
}

fn deinit(cmd: &mut Commands, enemy: &mut Enemy, enemy_ref: &mut ResMut<Enemies>, entity: Entity) {
...
fn update_position(time: Res<Time>, res_enemies: ResMut<Enemies>, query_player: Query<&Transform, With<player::PlayerSprite>>, mut query_enemy: Query<&mut Transform, (With<Enemy>, Without<player::PlayerSprite>)>) {
...
fn spawning(cmd: Commands, time: Res<Time>, mut res_enemies: ResMut<Enemies>) {
...
pub fn take_damage(cmd: &mut Commands, bullet_entity: Entity, enemy_entity: Entity, enemy: &mut Enemy, enemy_ref: &mut ResMut<Enemies>) -> bool {
...
fn update_gizmos(mut gizmos: Gizmos, res_enemies: ResMut<Enemies>) {
    gizmos.circle_2d(Vec2::ZERO, 10f32, ColorPalette::RED);
    gizmos.rect_2d(Vec2::ZERO, 0f32, Vec2 {x: res_enemies.max_range_spawn.x as f32, y: res_enemies.max_range_spawn.y as f32}, ColorPalette::WHITE);
    gizmos.rect_2d(Vec2::ZERO, 0f32, Vec2 {x: res_enemies.min_range_spawn.x as f32, y: res_enemies.min_range_spawn.y as f32}, ColorPalette::RED);
}